...
for i in "${!IPS[@]}"; do
  remote "${IPS[$i]}" "cd '$REMOTE_DIR' && tmux new-session -d -s bull-primary \"RUST_LOG=info ./target/release/node -vv run --keys deploy/node-${i}.json --committee deploy/committee.json --parameters deploy/parameters.json --store run/db-primary primary |& tee run/logs/primary-${i}.log\""
done
...
for i in "${!IPS[@]}"; do
  remote "${IPS[$i]}" "cd '$REMOTE_DIR' && tmux new-session -d -s bull-client \"RUST_LOG=info ./target/release/benchmark_client '${IPS[$i]}:3003' --size '$TX_SIZE' --rate '$RATE_SHARE' --nodes $TX_NODES |& tee run/logs/client-${i}-0.log\""
done
