...
```
They specify the number of primaries (`nodes`) and workers per primary (`workers`) to deploy, the input rate (tx/s) at which the clients submits transactions to the system (`rate`), the size of each transaction in bytes (`tx_size`), the active-adversary parameter (`faults`), and the duration of the benchmark in seconds (`duration`). The minimum transaction size is 9 bytes, this ensure that the transactions of a client are all different. The benchmarking script will deploy as many clients as workers and divide the input rate equally amongst each client. For instance, if you configure the testbed with 4 nodes, 1 worker per node, and an input rate of 1,000 tx/s (as in the example above), the scripts will deploy 4 clients each submitting transactions to one node at a rate of 250 tx/s.

Bullshark note: when `faults > 0`, all nodes remain online. Every round selects `faults` silent authorities using `BULLSHARK_ADVERSARY_SEED`; that round's steady leader is always selected, and the remaining `faults - 1` authorities are pseudo-random. Silent authorities suppress their header while continuing to receive messages. Before the run, the benchmark generates one wall-clock silence bitmap per client from the same schedule. `BULLSHARK_CLIENT_DURING_SILENCE=pause` (the default) stops that client's transactions and pauses local batch production in silent slots; `send` keeps client and batch traffic without restoring protocol participation. Slots default to `max_header_delay` milliseconds and can be set with `BULLSHARK_CLIENT_SILENCE_SLOT_MS`. For example: `BULLSHARK_ADVERSARY_SEED=42 BULLSHARK_CLIENT_DURING_SILENCE=pause fab local`. Fallback is still resolved with a `2f+1` causal-support threshold.
