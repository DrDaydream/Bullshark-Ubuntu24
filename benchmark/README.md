...
Bullshark note: when `faults > 0`, all nodes remain online. The primary command propagates `BULLSHARK_FAULTS`; both steady leaders are suppressed in each wave and one pseudo-random fallback leader is resolved with a `2f+1` causal-support threshold.
