...
class LogParser:
    ...
    def _parse_primaries(self, log):
        ...
        header_proposals = self._merge_results([
            [(d, self._to_posix(t)) for t, d in tmp]
        ])
        tmp = findall(
            r'\[(.*Z) .* Header committed round \d+ digest (\S+) leader (true|false)',
            log,
        )
        header_commits = {
            d: (self._to_posix(t), role == 'true') for t, d, role in tmp
        }
    ...
    def _header_latency_stats(self):
        ...
        leader_times = sorted(
            committed for committed, is_leader in self.header_commits.values()
            if is_leader
        )
        intervals = [b - a for a, b in zip(leader_times, leader_times[1:])]
        return (
            mean(leader) if leader else 0,
            mean(non_leader) if non_leader else 0,
            mean(all_headers) if all_headers else 0,
            mean(intervals) if intervals else 0,
            mean(ordered) if ordered else 0,
        )
    ...
    def result(self):
        ...
        leader_latency, non_leader_latency, all_header_latency, leader_interval, rule_order_latency = (
